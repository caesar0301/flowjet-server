...
Integration tests exercise the full HTTP / SSE / uvicorn stack (live-server
fixtures) or the ASGI app in-process against a real backend. The live-server
fixtures here spin up a real uvicorn process so the official ``openai`` client
exercises the same HTTP/SSE path production SDKs use.
"""
